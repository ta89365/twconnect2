...
const TUNE = {
  ...
  subTextSize:
    "text-[clamp(0.95rem,3.8vw,1.05rem)] sm:text-lg md:text-xl",
...
function formatDateDeterministic(
  lang: "jp" | "zh" | "en",
  iso: string,
) {
...
export default async function Home({
  ...
  const items: NavItem[] = rawItems
    .map((it) =>
      it?.href
        ? { ...it, href: addLangQuery(it.href, contentLang) }
        : it,
    )
  ...
  const secondaryLinks = [
    ...
    {
      label: "News",
      href: addLangQuery("/news", contentLang),
    },
    {
      label:
        contentLang === "jp"
  ...
  const channelLabel = (channel: "news" | "column"): string => {
    if (contentLang === "jp")
      return channel === "news" ? "ニュース" : "コラム";
    if (contentLang === "zh")
      return channel === "news" ? "新聞" : "專欄";
  ...
  const cnInvHref = cnInvestmentLanding?.slug
    ? `/${cnInvestmentLanding.slug}`
    : undefined;
  ...
  return (
    ...
    >
      {await (NavigationServer as any)({
        lang: spRaw?.lang as string,
      })}
...
function NewsSection({
  ...
  const t = {
    ...
    en: {
      title: "News & Column",
      sub: "Latest updates and columns in one place.",
      cta: "See all",
    },
  }[lang];
