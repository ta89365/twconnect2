...
import type { LucideIcon } from "lucide-react"; // ✅ 新增這行
...
export default async function CompanyPage({
  ...
  const navItems: NavItemOut[] = rawItems
    .filter((it): it is Required<Pick<NavItem, "label" | "href">> &
  ...
  const cName = lang === "jp" ? coFounder?.name?.jp : lang === "zh" ? coFounder?.name?.zh : coFounder?.name?.en;
  const cTitle = lang === "jp" ? coFounder?.title?.jp : lang === "zh" ? coFounder?.title?.zh : coFounder?.title?.en;
    ...
    <main className={`${TYPO.base} bg-[#1C3D5A] text-slate-100 tracking-[0.01em]`}>
      ...
      >
        ...
        <div className="pointer-events-none absolute inset-x-0 bottom-6">
          <div className={`${container} text-center`}>
            <span className={`${TYPO.h1} inline-block text-white`} style={{ textShadow: "0 2px 10px rgba(0,0,0,.45)" }}>
              {heroSmallLabel}
            </span>
          </div>
        </div>
      ...
      <section className="py-12 md:py-14">
        <div className={container}>
          ...
          <div className={`${TYPO.smallCaps} text-slate-300 text-center mt-1`}>MISSION / VISION / VALUE</div>
          ...
          <div className={`${card} p-4 md:p-5 mt-6`}>
            <div className={`${TYPO.h3}`}>
              <HeadingIcon icon={UserRound}>
                {missionLabel === "Our Purpose" ? "Mission" : (missionLabel === "經營理念" ? "使命" : "ミッション")}
              </HeadingIcon>
            </div>
            <div className="mt-3">
...
{/* Representative Message（純文字卡） */}
{(company?.repMessageLong || company?.repMessageShort) && (
  <section className="py-12 md:py-14">
    <div className={container}>
      <h2 className={`${TYPO.h2} text-center`}>{repLabel}</h2>
      <div className={`${TYPO.smallCaps} text-slate-300 text-center mt-1`}>MESSAGE</div>
      <div className="mx-auto mt-2 h-px w-12 bg-white/30" />
      ...
      <div className={`${card} p-4 md:p-5 mt-7`}>
        ...
        {company?.repMessageShort ? (
          <div
            ...
            style={{
              fontFamily: `'Bradley Hand', 'Segoe Script', 'Comic Sans MS', 'DFKai-SB', 'KaiTi', 'Yu Mincho', cursive`,
              fontWeight: 500,
